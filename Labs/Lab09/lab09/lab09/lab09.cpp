...
const unsigned int CITY_SIZE = 40;    //можно еще удалялку переписать
int am_st;
...
int main()
{
	...
	cout << "7 - add new element in the beginning of the list" << endl;
	cout << "8. exit" << endl;
	cout << endl;
	bool falg = false;
	for (;;)
	{
		...
		case 7: {
			if (falg == false)
				addToHead(setElement(), &head, &last);
			else
				addToHead(adr);
		}
			  break;
